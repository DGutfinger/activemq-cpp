...
void Socket::connect( const std::string& host, int port, int timeout ) {
    ...
    try{
        ...
        try {
			//no need to bind before connect, connect does the bind 
			// this bind tries to bind to the local port, which might be use - for example if the client and server are ont he same amchine

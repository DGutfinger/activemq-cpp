...
void TcpSocket::create() {
    ...
    try {
        ...
        checkResult(apr_socket_create(&this->impl->socketHandle,
            APR_UNSPEC, SOCK_STREAM, APR_PROTO_TCP, this->impl->apr_pool.getAprPool()));
...
void TcpSocket::bind(const std::string& ipaddress, int port) {
    ...
    try {
		...
		//get the local address family of the socket
		apr_sockaddr_t* localAddress;
		...
		// Create the Address Info for the desired address
		result = apr_sockaddr_info_get(	&impl->localAddress, host,
					family, (apr_port_t) port, flags, impl->apr_pool.getAprPool());
		checkResult(result);

		if( localAddress->family != impl->localAddress->family ) { 
			//this is IPv4 and our socket is IPv6  or vice versa, need to recreate the socket
			//delete the current socket 
	          apr_socket_shutdown(impl->socketHandle, APR_SHUTDOWN_READWRITE);
			  impl->socketHandle=NULL;
  			// Create the  socket wiht the correct address family
			checkResult(apr_socket_create(&this->impl->socketHandle, impl->localAddress->family,
					SOCK_STREAM, APR_PROTO_TCP, this->impl->apr_pool.getAprPool()));
			 // Initialize the Socket's FileDescriptor
			apr_os_sock_t osSocket = -1;
			apr_os_sock_get(&osSocket, this->impl->socketHandle);
			this->fd = new SocketFileDescriptor(osSocket);
		...
		if (result != APR_SUCCESS ) {
				close();
				throw SocketException(__FILE__, __LINE__, "TcpSocket::bind - %s", SocketError::getErrorString().c_str());
		}
...
void TcpSocket::connect(const std::string& hostname, int port, int timeout) {
    ...
    try {
		...
		apr_int32_t family= APR_UNSPEC;
		int flags= APR_IPV4_ADDR_OK;
		apr_status_t result = apr_sockaddr_info_get(&impl->remoteAddress, host.c_str(), family, (apr_port_t) port, flags, impl->apr_pool.getAprPool());
		checkResult(result);

		//check the type of local socket we have
		apr_sockaddr_t* localAddress;
        checkResult(apr_socket_addr_get(&localAddress, APR_LOCAL, impl->socketHandle));
		if( localAddress->family != impl->remoteAddress->family ) { 
		//this is IPv4 and our socket is IPv6  or vice versa, need to recreate the socket
			//delete the current socket
	          apr_socket_shutdown(impl->socketHandle, APR_SHUTDOWN_READWRITE);
			  impl->socketHandle=NULL;
			// Create the  socket wiht the correct address family
			checkResult(apr_socket_create(&this->impl->socketHandle, impl->remoteAddress->family, SOCK_STREAM, APR_PROTO_TCP, this->impl->apr_pool.getAprPool()));
			// ReInitialize the Socket's FileDescriptor
			apr_os_sock_t osSocket = -1;
			apr_os_sock_get(&osSocket, this->impl->socketHandle);
			this->fd = new SocketFileDescriptor(osSocket);
		}
...
void TcpSocket::listen(int backlog) {
    ...
    try {
        ...
        if (result != APR_SUCCESS) {
            close();
            throw SocketException(__FILE__, __LINE__, "Error on listen - %s", SocketError::getErrorString().c_str());
        }
    }
    DECAF_CATCH_RETHROW(decaf::io::IOException)
    DECAF_CATCH_EXCEPTION_CONVERT(Exception, decaf::io::IOException)
